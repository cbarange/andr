...
import { createFocusComputer, type Focus } from "./ui/focus";
...
import {
  gatherWood, lightFire, stokeFire, resolveEventChoice, tick, debugSetSeed,
  takeLoot, secureMine, setOutside,
  eatMeat, useMeds, steps,
  setPositions,
  flightFire, endFlight, steer, prestige, revealCells,
  isNetworkSafeAction, type PlayerAction,
} from "./sim/actions";
import { bestReadyWeapon, ENGAGE_RADIUS } from "./sim/combat";
import { caveSteps, executionerDungeon } from "./sim/dungeon";
...
async function boot(): Promise<void> {
  ...
  // ---- Détection de l'interactable le plus proche (étiquette + action E) — extrait dans
  //      ui/focus.ts (A6). Recalculé chaque frame par la boucle ; E déclenche `currentFocus.act()`.
  const computeFocus = createFocusComputer({
    getState: () => state,
    emit,
    self,
    selfEngagedEnc,
    ...
    getChopCooldown: () => chopCooldown,
    interactFire,
    chopTree,
    isCineActive: () => cine.active,
    // RF5 — pénétrer par le SAS : porte alien ANCRÉE AU SAS + PAS SCRIPTÉ + fondu -> commit au noir.
    startShipEntryCine: (commit) => startThresholdCine("in", "ship", true, commit, shipInterior.entrance()),
    player,
    village,
    stranger,
    cabin,
