...
// SEUIL D'ENTRÉE (écran-titre) — sauté par défaut en webdriver (sinon il bloquerait les 19 autres
// e2e) ; on le FORCE via ?title=1 pour vérifier l'affichage + le démarrage de la partie.
test("ÉCRAN-TITRE : le seuil s'affiche (forcé) et « commencer » lance la partie", async ({ page }) => {
  await page.goto("/?title=1");
  await page.waitForFunction(() => window.__game?.ready === true, undefined, { timeout: 60_000 });
  await expect(page.locator("#titleScreen")).toBeVisible();
  await expect(page.locator("#titleScreen .titleName")).toContainText("A Dark Room");
  await expect(page.locator("#titleStart")).toHaveText("commencer"); // localStorage e2e vierge -> partie neuve
