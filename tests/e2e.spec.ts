...
test("la musique suit l'état du feu et le menu Son règle les volumes", async ({ page }) => {
  ...
  await expect(page.locator("#sfxToggles input[type=checkbox]")).toHaveCount(7);
...
test("les sites sont posés et passent en LOD silhouette → détail (P5)", async ({ page }) => {
  ...
  expect(s0!.placed).toBe(17); // 4 grottes + 5 maisons + 2 villes + 3 mines + marais + épave + cuirassé
  expect(s0!.types).toBe(9);
