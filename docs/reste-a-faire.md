...
- [~] **A6 — refactor `main.ts`** *(EN COURS — 5 extractions livrées, iso-comportement, un commit
  ...
  état propre) · **A6.3** écran-titre -> `ui/titleScreen.ts`. **main.ts : 2704 -> 1735 lignes (−969, −36 %).**
  Chaque extraction validée par typecheck + 300 tests + 21 e2e. · **A6.4** vues de dialogue + révélation + plomberie -> `ui/dialogues.ts` (~500 l., identité `eventView`/`rootView` préservée via des membres stables). · **A6.5** computeFocus -> `ui/focus.ts` (~360 l., + correctif : la carte-monde passe par ACCESSEUR dans gameHooks — un `let` régénéré au prestige//seed était capturé par valeur). **Reste** : `reflectState` (~345 l.), wiring audio (~140 l.).
